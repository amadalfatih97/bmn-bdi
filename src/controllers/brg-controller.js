...
module.exports ={
    ...
    getDataBarang(req,res){
        pool.getConnection(function(err, connection) {
            ...
            connection.query(
                `
                SELECT * FROM barangs WHERE barangs.aktif = 1;
                `
    ...
    getDataBarangByID(req,res){
        ...
        pool.getConnection(function(err, connection) {
            ...
            connection.query(
                `
                SELECT * FROM barangs WHERE kode = ?;
                `
            , [id],
            function (error, results) {
                if(error) throw error;  
    ...
    addDataBarang(req,res){
        let data = {
            kode : req.body.nama,
            namabarang : req.body.umur,
            idsatuan : req.body.alamat,
            stok : req.body.jabatan,
            lokasi : req.body.jabatan,
            ket : req.body.jabatan
        }
        pool.getConnection(function(err, connection) {
            if (err) throw err;
            connection.query(
